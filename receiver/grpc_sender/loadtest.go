...
func makeRequests(hosts int, plugins int, values int, hostStart int) []*pb.Message {

	out := make([]*pb.Message, hosts*plugins)
	...
	for host := 0; host < hosts; host++ {
		for plugin := 0; plugin < plugins; plugin++ {
			...
			request := &pb.Message{
				Data: make([]*pb.Metric, 0, values),
			}

			for value := 0; value < values; value++ {
				request.Data = append(request.Data, &pb.Metric{
					...
					Timestamps: []uint32{uint32(time.Now().Unix())},
